...
void exit(int status)
{
  ...
  if (p->is_batchproc)
  {
    ...
    if (p->stime < batch_start)
      batch_start = p->stime;
    batchsize--;
    turnaround += (p->endtime - p->stime);
    waiting_tot += p->waittime;
    completion_tot += p->endtime;
    if (p->endtime > completion_max)
      completion_max = p->endtime;
    if (p->endtime < completion_min)
      completion_min = p->endtime;
    if (batchsize == 0)
    {
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    else
    {
      for (p = proc; p < &proc[NPROC]; p++)
      {
        if ((sched_policy != SCHED_NPREEMPT_FCFS) && (sched_policy != SCHED_PREEMPT_RR))
          break;
        acquire(&tickslock);
        xticks = ticks;
        release(&tickslock);
...
{
  initsleeplock(&lock_delete, "delete lock");

  initsleeplock(&lock_insert, "insert lock");

  initsleeplock(&lock_print, "print lock");
  ...
  {
    initsleeplock(&buffer[i].lock, "buffer element lock");
  }
}
...
void cond_produce(int value)
{
  ...
  {
    // printf("Waitinf for index : %d, the value here is : %d\n", index, buffer[index].x);
    cond_wait(&buffer[index].deleted, &buffer[index].lock);
  }
  buffer[index].x = value;
  // printf("The value at buffer[%d] is : %d\n", index, value);
  buffer[index].full = 1;
...
int cond_consume(void)
{
  ...
  acquiresleep(&buffer[index].lock);
  // printf("Acquired this index's lock\n");
  while (!buffer[index].full)
  {
    printf("Waiting at index : %d\n", index);
    cond_wait(&buffer[index].inserted, &buffer[index].lock);
  }
  // printf("Value of buffer[%d] is : %d\n", index, buffer[index].x);
  int v = buffer[index].x;
  buffer[index].full = 0;
...
int sem_consume(void)
{
  ...
  sem_wait(&full);
  sem_wait(&con);
  int v = buffer_sem[nextc];
  nextc = (nextc + 1)%SEM_SIZE;
  printf("%d ", v);
  sem_post(&con);
  sem_post(&empty);
  return v;
...
int barrier_alloc(){
  ...
  initsleeplock(&lock, "barrier lock init");
  acquiresleep(&lock);
  if(!barrierInitialized){
    for(int i=0;i<10;i++){
      barrier_array[i].isFree = 1;
      initsleeplock(&barrier_array[i].lock, "barrier element lock");
    }
    barrierInitialized = 1;
  }
  releasesleep(&lock);

  for(int i=0;i<10;i++){
    acquiresleep(&barrier_array[i].lock);
    if(barrier_array[i].isFree ==1){
      barrier_array[i].isFree = 0;
      barrier_array[i].count = 0;
      releasesleep(&barrier_array[i].lock);
      return i;
    }
    releasesleep(&barrier_array[i].lock);
  }
  printf("Barrier array is full. No space...\n");
  exit(0);
  return -1;
}

void barrier(int barrier_instance, int barrier_id, int n){
  printf("%d: Entered barrier#%d for barrier array id %d\n",myproc()->pid,barrier_instance,barrier_id);

  acquiresleep(&barrier_array[barrier_id].lock);
  barrier_array[barrier_id].count++;
  if(barrier_array[barrier_id].count == n){
    barrier_array[barrier_id].count = 0;
    cond_broadcast(&barrier_array[barrier_id].cv);
  }
  else{
    cond_wait(&barrier_array[barrier_id].cv,&barrier_array[barrier_id].lock);
  }
  releasesleep(&barrier_array[barrier_id].lock);
...
void barrier_free(int barrier_id){
  acquiresleep(&barrier_array[barrier_id].lock);
  barrier_array[barrier_id].isFree = 1;
  releasesleep(&barrier_array[barrier_id].lock);
}